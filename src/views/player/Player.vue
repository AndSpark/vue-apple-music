		...
		<div class="player pointer" ref="player">
			...
			<div class="top-cover" ref="topCover">
				...
				<div class="title" ref="title">
					<p class="elellipsis" :class="{ 'video-ar': this.song.mv }">
						{{ song.name }}
					</p>
					<span
					...
					></span>
					<span
						class="iconfont icon-playerback_filled a-right"
						@click.stop="nextSong"
					></span>
				</div>
			...
			<div class="play-btn" ref="playBtn">
				<span
					class="iconfont icon-playerback_filled"
					@click="backSong"
					ref="backSong"
				></span>
				<span
				...
				></span>
				<span
					class="iconfont icon-playerback_filled aRight"
					@click="nextSong"
					ref="nextSong"
				></span>
			</div>
	...
	methods: {
			...
			if (!this.isMove) {
				...
				if (this.song.mv) {
					if (!this.origin) {
						...
						this.$refs.video.style.top = `${0.7 +
							(this.H * 0.5 - this.W * 0.285 - 0.7) * rat}rem`
						this.$refs.video.style.left = `${-1.5 + rat * 1.5}rem`
					}
				} else {
					if (!this.origin) {
						...
						this.$refs.img.style.width = `${3.5 +
							(this.H * 0.8 - 3.5) * rat}rem`
						this.$refs.img.style.top = `${0.6 + (this.H * 0.1 - 0.6) * rat}rem`
						this.$refs.img.style.left = `${-1.5 +
							(this.W * 0.5 + 1.5 - this.H * 0.8 * 0.5) * rat}rem`
					}
				}
				...
				this.$refs.player.style.borderRadius = `${2 * rat}rem ${2 * rat}rem 0 0`
		...
		timeupdate() {
			if (!this.isMove) {
				...
				this.$refs.i.style.width = `${(this.currentTime / this.duration) *
					100}%`
				this.$refs.b.style.left = `${(this.currentTime / this.duration) * 100}%`
			}
		},
		ended() {
			this.playing = false
		...
		listAnimation(v) {
			if (this.song.mv) {
				if (!v) {
					...
					this.$refs.video.style.top = `${0.7 +
						(this.H * 0.5 - this.W * 0.285 - 0.7)}rem`
			...
			if (!v) {
				...
				this.$refs.img.style.left = `${-1.5 +
					(this.W * 0.5 + 1.5 - this.H * 0.8 * 0.5)}rem`
				setTimeout(() => {
					this.$refs.img.style.transition = null
				}, 300)
			}
