  ...
  >
    ...
    <div class="player pointer" ref="player">
      ...
      <div class="top-cover" ref="topCover">
        <img :src="song.al.picUrl+ '?param=500y500' " ref="img" @load="refresh" />
      ...
      <div class="bottom-option" ref="btOp">
        <span class="iconfont icon-Message" @click="lyricClick"></span>
...
export default {
  ...
  computed: {
    ...
    FMMode() {
      return this.$store.state.FMMode;
    },
    isEnd() {
  ...
  methods: {
    ...
    //图片滑动缩放效果,为什么使用定位不使用动画，
    //因为transform-origin的定位问题，图片的动画效果会超出边界
    scrolling(y) {
      if (!this.isMove) {
        let rat = y / this.topRem;
        if (!this.origin) {
          this.$refs.img.style.height = `${3.5 + 0.9 * rat}rem`;
    ...
    // 图片旋转
    valiRotate() {
      let h = parseInt(this.$refs.img.style.height);
    ...
    timeupdate() {
      if (!this.isMove) {
        this.realCurTime = this.$refs.myMusic.currentTime.toFixed(2) * 1;
        this.currentTime = this.$refs.myMusic.currentTime.toFixed(0) * 1;
        this.duration = this.$refs.myMusic.duration.toFixed(0) * 1;
        this.leftTime =
          this.duration - this.$refs.myMusic.currentTime.toFixed(0) * 1;
    ...
    touchmove(e) {
      ...
      this.currentTime = this.rate * this.$refs.myMusic.duration;
      this.leftTime = this.duration - this.currentTime;
    },
    touchend(e) {
      this.isMove = false;
      this.$refs.myMusic.currentTime = this.rate * this.$refs.myMusic.duration;
      this.currentTime = this.$refs.myMusic.currentTime.toFixed(0);
    ...
    listAnimation(v) {
      if (!v) {
        ...
        this.$refs.img.style.height = `${4.4}rem`;
        ...
        this.$refs.img.style.transition = ".3s";
        this.$refs.img.style.height = `${3.5 + (this.H * 0.8 - 3.5)}rem`;
