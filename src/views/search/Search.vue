<template>
  <div class="search-page">
    <div class="hot-search" v-if="showHot">
      <h2>热门搜索</h2>
      <p v-for="(item, index) in hots" :key="index" @click="hotSearch(item.first)">{{item.first}}</p>
    </div>
    <div class="search-item" v-for="(title, index) in order" :key="index">
      ...
      <div class="content" v-for="(item) in result[title]" :key="item.id">
        <div v-if="title == 'artists'" class="artist" @click="searchClick(item,title)">
        ...
        <div v-else class="other" @click="searchClick(item,title)">
          <img :src=" item.coverImgUrl + '?param=100y100'" @load="scrollRefresh" />
          <div>
            <p class="elellipsis">{{item.name}}</p>
...
export default {
  ...
  methods: {
    ...
    searchClick(item, title) {
      switch (title) {
        case "artists":
          this.$store.commit("sendArId", item.id);
