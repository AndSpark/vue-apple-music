    ...
    <div class="comment-main">
      <div class="comments">
        <div class="title-option">
          <span @click="titleClick(true)" :class="{hot}">热门</span>
          <span @click="titleClick(false) " :class="{hot:!hot}">最新</span>
        </div>
        <div class="comment" v-for="(comment, index) in showComments" :key="index">
          <div class="top">
            <div class="left">
              <img :src="comment.avatar + '?param=100y100'" @load="scrollRefresh" />
            </div>
            <div class="right">
              <div>
                <p>{{comment.name}} :</p>
          ...
          <div class="bottom">
            ...
            <p @click="likeClick(comment,showComments)">
              <span class="iconfont icon-heart" :class="{liked:comment.liked}"></span>
              {{comment.likedCount}}
            </p>
          </div>
        </div>
...
<script>
import { getComment_, getHotComment_, likeComment_ } from "@/network/content";
export default {
  name: "Comment",
  components: {},
  props: {},
  ...
  methods: {
    ...
    gethotComments() {
      return getHotComment_(this.song.id, 30, ++this.hotCurPage).then((res) => {
    ...
    reply(comment) {
      this.$emit("show-input", { comment, song: this.song });
    },
