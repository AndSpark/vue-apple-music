<template>
  <div class="main-bar">
    <div class="main-bar-p" ref="mainBar">
      ...
      <div class="block" ref="broadcast" @click="broadcastClick">
...
export default {
  ...
  computed: {
    commentPath() {
      if (
        this.$parent.$refs.player &&
        (!this.$parent.$refs.player.isRotate ||
          !this.$parent.$refs.player.origin)
      )
        return false;
  ...
  methods: {
    ...
    broadcastClick() {
      this.removeRed();
      this.$tap("broadcast");
      if (this.$route.path == "/broadcast") this.$emit("scrollTop");
    },
    showInput() {
